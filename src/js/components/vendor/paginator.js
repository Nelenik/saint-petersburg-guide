	...
	this.core = {
		...
		renderBtns() {
			...
			if (enablePrevAndNext) {
				let prev = this.createControlBtn(
					btnsInnerObj.prev,
					prevInnerHtml,
					prevClass
				);
				leftWrap.append(prev);
				let next = this.createControlBtn(
					btnsInnerObj.next,
					nextInnerHtml,
					nextClass
				);
			...
			if (enableFirstAndLast) {
				let first = this.createControlBtn(
					btnsInnerObj.first,
					firstInnerHtml,
					startClass
				);
				leftWrap.prepend(first);
				let last = this.createControlBtn(
					btnsInnerObj.end,
					lastInnerHtml,
					endClass
				);
			...
			let newUrl =
				this.currentPage == 1 ? baseUrl : baseUrl + `?page=${this.currentPage}`;
		...
		ratePagBtnsInner() {
			let {
				totalPages: pages,
				visibleAtLeft: lBtns,
				visibleAtRight: rBtns,
				maxCountOfVisible: maxBtns,
				enableDots: dots,
				enablePrevAndNext,
				enableFirstAndLast,
			} = this.options;
		...
		createPaginBtn(pageNum) {
			...
			const paginBtn = this.createHtml({
				...
				attributes: {
					href: pageNum > 1 ? `${hrefValue}?page=${pageNum}` : `${hrefValue}`,
					'data-page': pageNum,
				},
				text: pageNum,
			});
