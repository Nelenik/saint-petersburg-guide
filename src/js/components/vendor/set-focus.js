	...
	const core = {
		init() {
			const defaults = {
				...
				focusedPost: document.querySelector(
					'a, input:not([disabled]), button:not([disabled]), [tabindex]'
				), //если не указан в настройках, по умолчанию фокус перейдет к первому на странице элементу
			};
			this.options = Object.assign(defaults, options);
			this.isFirstOnPage = this.options.isFirstOnPage;
			this.container = this.isSelector(blockToSetFocus)
				? document.querySelector(blockToSetFocus)
				: blockToSetFocus;
			this.prev = this.isSelector(this.options.focusedPrev)
				? document.querySelector(this.options.focusedPrev)
				: this.options.focusedPrev;
			this.post = this.isSelector(this.options.focusedPost)
				? document.querySelector(this.options.focusedPost)
				: this.options.focusedPost;
		...
		manageTabNav() {
			...
			window.addEventListener(
				'resize',
				this.throttle(this.setTabNav, 100).bind(this)
			);
		},
		//настройка фокуса
		setTabNav() {
			...
			document.addEventListener('keydown', (e) => {
				...
				if (this.isFirstOnPage) {
					this.container.addEventListener(
						'focus',
						this.setFocus(e.shiftKey).bind(this),
						{ once: true }
					);
				}
				let currentFocused = document.activeElement;
				currentFocused.addEventListener(
					'blur',
					this.setBlur(e.shiftKey).bind(this),
					{ once: true }
				);
			});
		},
		...
		setBlur(isShift) {
			return function (e) {
				...
				if (!isShift && currentFocused === before) {
					...
					isShift
						? currentFocused === this.firstArrEl
							? before.focus()
							: this.focusedArr[currentIndex - 1].focus()
						: currentFocused === this.lastArrEl
						? after.focus()
						: this.focusedArr[currentIndex + 1].focus();
				}
			};
		...
		getFocusedArr() {
			...
			this.focusedArr = Array.from(this.focusedArr).filter(
		...
		getFocusableElems(el) {
			...
			let result = elemsArray.filter(
				(i) => !i.hasAttribute('tabindex') || i.getAttribute('tabindex') >= 0
			);
			return result;
		},
